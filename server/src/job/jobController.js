...
const createJob = async (req, res) => {
	try {
		...
		const hrUser = await prisma.hrUser.findUnique({
			where: { id: BigInt(hrId), isDeleted: false },
		});
		...
		const result = processedResponse(newJobPost);
...
//JobPosts my All list

const myJobList = async (req, res) => {
	const { id, role } = req.user;
	try {
		const jobPosts = await prisma.jobPost.findMany({
			where: {
...
const toggleJobPostActive = async (req, res) => {
	try {
		...
		res.status(200).json({
			message: `JobPost is now ${updatedJobPost.isActive ? "active" : "inactive"}.`,
			jobPost: response,
		});
	} catch (error) {
